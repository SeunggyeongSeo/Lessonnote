...
import { isConfigured, loadState, saveState, subscribeState, authStore } from "./supabaseClient";
import {
  ...
  UserPlus, MailCheck, ShieldCheck, Search, CornerDownRight, Paperclip, Archive, RotateCcw, ChevronUp, Move, AlertTriangle, Tablet
} from "lucide-react";
...
function DiaryView({ data, student, canEdit, defaultTeacherId, me, api }) {
  ...
  return (
    <div>
      ...
      {entries.map((en, i) => { const cs = en.commentList || []; const ccount = cs.length + cs.reduce((a, c) => a + ((c.replies || []).length), 0); const isOpen = openComments === en.id; return (
        <div key={en.id} className="dc-card dc-enter" style={{ padding: 16, marginBottom: 16, animationDelay: `${Math.min(i, 6) * 0.05}s` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
            <div style={{ width: 38, height: 38, borderRadius: 13, background: "linear-gradient(140deg,#6A4C7A,#4D3759)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff" }}><Music2 size={17} /></div>
            <div style={{ flex: 1 }}><div style={{ fontSize: 13.5, fontWeight: 700 }}>{tName(en.teacherId)} 선생님</div><div style={{ fontSize: 11.5, color: "var(--ink-soft)" }}>{en.date}</div></div>
          ...
          {en.media.length > 0 && <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(en.media.length, 3)},1fr)`, gap: 8, marginBottom: 13 }}>{en.media.map((m, j) => (<div key={j} className="dc-thumb" style={{ background: m.url ? "#000" : MEDIA_HUES[m.h], overflow: "hidden" }}>{m.url ? (m.t === "video" ? <video src={m.url} muted style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <img src={m.url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />) : <span>{m.e}</span>}{m.t === "video" && <div className="dc-play"><span><Play size={15} fill="#6A4C7A" color="#6A4C7A" /></span></div>}<div style={{ position: "absolute", top: 6, left: 6 }}>{m.t === "video" ? <Video size={13} color="#fff" /> : <Camera size={13} color="#fff" />}</div></div>))}</div>}
      ...
      {compose && <ComposeDiary student={student} defaultTeacherId={defaultTeacherId} onSave={(e) => { api.addDiary(e); setCompose(false); }} onClose={() => setCompose(false)} />}
    </div>
  );
}
...
function ComposeDiary({ student, defaultTeacherId, initialText, initialTitle, onSave, onClose }) {
  ...
  const onPick = (e) => { const files = Array.from(e.target.files || []); const items = files.map(f => ({ t: f.type.startsWith("video") ? "video" : "photo", url: URL.createObjectURL(f), name: f.name })); setMediaList(m => [...m, ...items]); e.target.value = ""; };
  const removeMedia = (i) => setMediaList(m => m.filter((_, k) => k !== i));
  const save = () => { if (!title.trim()) return; const dd = new Date(); onSave({ id: uid("d"), studentId: student.id, teacherId: defaultTeacherId, date: `${dd.getMonth() + 1}월 ${dd.getDate()}일`, title, text, media: mediaList, likes: 0, liked: false, commentList: [] }); };
  ...
  return (<Sheet title={`${student.name} 알림장 작성`} onClose={onClose}>
    ...
    <button className="dc-btn" onClick={() => fileRef.current && fileRef.current.click()} style={{ width: "100%", padding: "14px 0", borderRadius: 14, background: "#F8F1E6", color: "var(--plum)", fontSize: 13.5, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 12, border: "1.5px dashed #D8B79E" }}><Paperclip size={17} /> 갤러리에서 사진·영상 첨부</button>
    {mediaList.length > 0 && <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 7, marginBottom: 10 }}>{mediaList.map((m, i) => (<div key={i} className="dc-thumb" style={{ background: "#000", overflow: "hidden" }}>{m.t === "video" ? <video src={m.url} muted style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <img src={m.url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />}{m.t === "video" && <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ width: 26, height: 26, borderRadius: "50%", background: "rgba(255,255,255,.85)", display: "flex", alignItems: "center", justifyContent: "center" }}><Play size={13} fill="#6A4C7A" color="#6A4C7A" /></span></div>}<button className="dc-btn" onClick={() => removeMedia(i)} style={{ position: "absolute", top: 3, right: 3, width: 18, height: 18, borderRadius: "50%", background: "rgba(45,40,51,.6)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", padding: 0 }}><X size={11} /></button></div>))}</div>}
    {mediaList.length > 0 && <div style={{ fontSize: 11.5, color: "var(--ink-soft)", marginBottom: 14 }}>사진 {photos}장 · 영상 {vids}개 첨부됨</div>}
    <PrimaryBtn onClick={save}><Send size={16} /> 알림장 등록</PrimaryBtn>
  </Sheet>);
}
...
export default function App() {
  ...
  const api = {
    ...
    addDiary: (e) => D(d => { d.diary.unshift(e); const stu = d.students.find(s => s.id === e.studentId); d.notifications.unshift({ id: uid("n"), academyId: stu && stu.academyId, aud: { kind: "parentOf", studentId: e.studentId }, type: "diary", text: `📒 새 알림장 · ${e.title.slice(0, 22)}`, time: hhmm(), readBy: [] }); return d; }),
      ...
      <div className="dc-phone">
        ...
        {notifOpen && (
          <Sheet title="알림" onClose={() => setNotifOpen(false)}>
            ...
            {myNotifs.map(n => { const isUnread = !n.readBy.includes(me.id); const goTab = n.type === "chat" ? "chat" : n.type === "diary" ? "diary" : n.type === "pay" ? (me.role === "admin" ? "manage" : "pay") : "diary"; return (
              <button key={n.id} className="dc-btn" onClick={() => { api.markNotifRead(n.id, me.id); setTab(goTab); setNotifOpen(false); }} style={{ width: "100%", display: "flex", alignItems: "flex-start", gap: 11, padding: "12px 12px", marginBottom: 8, borderRadius: 14, background: isUnread ? "#FFF6EC" : "#fff", border: `1px solid ${isUnread ? "#F4D7C2" : "var(--line)"}`, textAlign: "left" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13.5, fontWeight: isUnread ? 700 : 400, color: "var(--ink)", lineHeight: 1.5 }}>{n.text}</div><div style={{ fontSize: 11, color: "var(--ink-soft)", marginTop: 3 }}>{n.time}</div></div>
              </button>
            ); })}
          </Sheet>
        )}
